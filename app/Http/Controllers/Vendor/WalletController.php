...
class WalletController extends Controller
{
    public function index()
    {
        ...
        return view('vendor-views.wallet.index', compact('withdraw_req', 'wallet'));
    }
    ...
    public function close_request($id)
    {
        $wr = WithdrawRequest::where('vendor_id', Helpers::get_vendor_id())
            ->findOrFail($id);

        if ($wr->approved == 0) {
            StoreWallet::where('vendor_id', Helpers::get_vendor_id())
                ->decrement('pending_withdraw', $wr->amount);
        }
