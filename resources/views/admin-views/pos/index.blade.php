...
<head>
    ...
    @php($business_name = optional(\App\Models\BusinessSetting::where(['key'=>'business_name'])->first())->value)
    <!-- Title -->
...
<body class="footer-offset">
                    ...
                    <div class="col-md-8 card padding-y-sm card ">
                        <div class="card-header d-flex flex-wrap  w-100">
                            <div class="row w-100 justify-content-around">
                                ...
                                <div class="col-sm-6 col-12 mb-2">
                                    <select name="module_id" class="form-control js-select2-custom" data-filter-key="module_id" title="{{translate('messages.select')}} {{translate('messages.modules')}}">
                                ...
                                <div class="col-sm-6 col-12 mb-2">
                                    <select name="store_id" id="store_select" data-filter-key="store_id" data-placeholder="{{translate('messages.select')}} {{translate('messages.store')}}" class="js-data-example-ajax form-control">
                                        @if($store)
    ...
    $(document).on('ready', function () {
        ...
        // =======================================================
        // 1. Initialize generic Select2 elements (excluding the custom Ajax one)
        $('.js-select2-custom').not('#customer').each(function() {
            // Only init if HSCore exists to prevent errors
            if ($.HSCore && $.HSCore.components.HSSelect2) {
                $.HSCore.components.HSSelect2.init($(this));
            }
        });

        // 2. Initialize the specific Ajax Customer Select2
        $('#customer').select2({
            ajax: {
                url: "{{ route('admin.pos.customers') }}",
                dataType: 'json', // Explicitly state we expect JSON
                delay: 250, // Wait 250ms before sending request (saves server load)
                data: function(params) {
                    return {
                        q: params.term,
                        page: params.page
            ...
            // Note: '__port' was removed because it is invalid.
        });
...
lng: {{ (float) ($store['longitude'] ?? 90.360587) }}
    ...
    <!-- IE Support -->
    <div id="ie-polyfill"
        data-src="{{ asset('public/assets/admin/vendor/babel-polyfill/polyfill.min.js') }}">
    </div>
