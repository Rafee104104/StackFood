        ...
        <div class="col-lg-8 mb-3 mb-lg-0">
                ...
                <div class="card-body">
                    ...
                    <div class="media">
                        ...
                        <div class="avatar avatar-xl mr-3 cursor-pointer"
                            ...
                            @php($item = $detail->item)

                            <img class="img-fluid"
                                src="{{ optional($item)->image
            ...
            : asset('assets/admin/img/160x160/img2.jpg') }}"
                    ...
                    <a class="avatar avatar-xl mr-3"
                        href="{{ $detail->item ? route('admin.item.view', $detail->item->id) : '#' }}">

                        <img class="img-fluid"
                            src="{{ $detail->item && $detail->item->image
                ? asset('storage/product/' . $detail->item->image)
                : asset('assets/admin/img/160x160/img2.jpg') }}"
                            alt="Image Description">
                    ...
                    <div class="media-body">
                        <div class="row">
                            <div class="col-md-6 mb-3 mb-md-0">
                                <strong>
                                    {{ Str::limit(optional($detail->item)->name ?? '', 20, '...') }}
                ...
                <!-- End Media -->
                <hr>
                @if(isset($detail->item_campaign_id) && $detail->status)
                <?php
                if (!$editing) {
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title h4" id="mySmallModalLabel">{{ translate('messages.reference') }}
                    {{ translate('messages.code') }} {{ translate('messages.add') }}
                </h5>
...
<script>
    function addDeliveryMan(id) {
        $.ajax({
            type: "GET",
            url: '{{ url(' / ') }}/admin/order/add-delivery-man/{{ $order['
            id '] }}/' + id,
            success: function(data) {
                location.reload();
                console.log(data)
    ...
    return function() {
        ...
        @if($parcel_order)
            ...
            infowindow.setContent(
                ...
                "<img style='max-height:40px;width:auto;' src='{{ $order->customer && $order->customer->image
                    ? asset('storage/profile/'.$order->customer->image)
                    : asset('assets/admin/img/160x160/img1.jpg') }}'>" +
                "</div>" +
                "<div style='float:right; padding: 10px;'>" +
                "<b>{{ optional($order->customer)->f_name }} {{ optional($order->customer)->l_name }}</b><br />" +
                "{{ $address['address'] ?? '' }}" +
                "</div>"
            );
        ...
        @else
            ...
            infowindow.setContent(
                "<div style='float:left'>" +
                "<img style='max-height:40px;width:auto;' src='{{ $order->store && $order->store->logo
                    ? asset('storage/store/'.$order->store->logo)
                    : asset('assets/admin/img/160x160/img1.jpg') }}'>" +
                "</div>" +
                "<div class='text-break' style='float:right; padding: 10px;'>" +
                "<b>{{ Str::limit(optional($order->store)->name, 15, '...') }}</b><br />" +
    ...
    $(document).ready(function() {
        ...
        function initializegLocationMap() {
            ...
            var dmmarker = new google.maps.Marker({
                position: new google.maps.LatLng({
                    {
                        $order->dm_last_location['latitude']
                    }
                }, {
                    {
