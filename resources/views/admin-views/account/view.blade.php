        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h3 class="h3 mb-0  ">{{$account_transaction->store?translate('messages.store'):translate('messages.deliveryman')}} {{translate('messages.info')}}</h3>
                </div>
                <div class="card-body">
                    <div class="col-md-8 mt-2">
                        <h4>{{translate('messages.name')}}: {{$account_transaction->store ? $account_transaction->store->name : $account_transaction->deliveryman->f_name.' '.$account_transaction->deliveryman->l_name}}</h4>
                        <h6>{{translate('messages.phone')}}  : {{$account_transaction->store ? $account_transaction->store->phone : $account_transaction->deliveryman->phone}}</h6>
                        <h6>{{translate('messages.cash_in_hand')}} : {{\App\CentralLogics\Helpers::format_currency($account_transaction->store ? $account_transaction->store->vendor->wallet->collected_cash : $account_transaction->deliveryman->wallet->collected_cash)}}</h6>
                    </div>
                </div>
            </div>
        </div>
